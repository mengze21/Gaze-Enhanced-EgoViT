# -------------------------------------------------------------------------
# input shape is x [B, 32, 3, 120, 160], features shape is [B, 32, 3, 2048]
# fix the bug, which batch size is only 1
# improve merging class token map
# update frome newT3
# use shared weights to reduce memory usage
# -------------------------------------------------------------------------
...
class DCTG(nn.Module):
    ...
    def forward(self, input_features):
        ...
        # mean_features = mean_features.squeeze(3)
...
class PADM(nn.Module):
    ...
    def partion_xcls(self, x_cls_group):
        ...
        x_cls_group = x_cls_group.view(B, G, C, D // window_size[0], window_size[0], H // window_size[1],
                                       window_size[1], W // window_size[2], window_size[2])
        # print(f"x_cls_group shape after view: {x_cls_group.shape}")
        x_cls_windows = x_cls_group.permute(0, 1, 3, 5, 7, 4, 6, 8, 2).contiguous().view(B, G, D // window_size[0],
                                                                                         H // window_size[1],
                                                                                         W // window_size[2],
                                                                                         reduce(mul, window_size), C)
    ...
    def merge_cls1(self, x):
        ...
        diag_mask = torch.eye(G, dtype=torch.bool)
        scores[diag_mask] = 0

        # # Sum over all spatial dimensions to get αg,s,g′
        # alpha_gsgp = scores.sum(dim=1)  # Shape: [G, G]
        ...
        # alpha = scores.sum(dim=1)  # Shape: [G]
        ...
        x_cls = xcls_weighted.view(C, D, H, W)
    ...
    def merge_cls(self, x_cls):
        ...
        # print(f"x_cls_flat shape: {x_cls_flat.shape}")
        # Compute L2 norms
        l2norms = torch.linalg.norm(x_cls_flat, ord=2, dim=2, keepdim=True)
        # print(f"l2norms shape: {l2norms.shape}")
        # Compute norms
        ...
        # print(f"dot_products shape: {dot_products.shape}")
        # Mask diagonal to exclude self-comparison
        diag_mask = torch.eye(G, dtype=torch.bool).unsqueeze(0).repeat(B, 1, 1)
        dot_products[diag_mask] = 0
        # print(f"dot_products shape after masking: {dot_products.shape}")
        # score per group
        alpha = dot_products.sum(dim=-1)
        # print(f"scorse shape after sum: {alpha.shape}")
        # total score of all groups
        alpha_gs = alpha.sum(dim=1)
        # print(f"alpha_total shape: {alpha_gs.shape}")
        ...
        # print(f"xcls_weighted shape: {xcls_weighted.shape}")

        # Reshape to original shape (B,C, D, H, W)
        x_cls_weighted = xcls_weighted.view(B, C, D, H, W)
        # print(f"x_cls_weighted shape: {xcls_weighted.shape}")

        return x_cls_weighted

        # # Compute alpha for each pair of groups (excluding self-pairing)
        # for g in range(group):
        #     for gp in range(group):
        #         if g != gp:
        #             g_norm = norms[:, g, :, :]  # Shape [batch, 1, spatial]
        #             gp_norm = norms[:, gp, :, :]  # Shape [batch, 1, spatial]
        #             # Dot products for all spatial locations
        #             print(f"x_cls_flat[:, g] shape: {x_cls_flat[:, g].shape}")
        #             print(f"x_cls_flat[:, gp] shape: {x_cls_flat[:, gp].shape}")
        #             dot_products = torch.einsum('bki,bkj->bij', x_cls_flat[:, g], x_cls_flat[:, gp])
        #             print(f"dot_products shape: {dot_products.shape}")
        #             # sum over spatial dimensions
        #             dot_products = dot_products.sum(dim=-1)
        #             print(f"dot_products shape after sum: {dot_products.shape}")
        #             # Normalizing the dot products
        #             alpha[:, g, gp] = dot_products / (g_norm * gp_norm.transpose(-2, -1))
    ...
    def forward11(self, x_xcl_s1):
        ...
        # print(f"x_group shape after permute: {x_group.shape}")
        ...
        # print(f"x_cls_group shape after padding: {x_cls_group.shape}")
    ...
    def forward(self, x):
        """Input x is x_cls_s1.
        shape is (B, G, C, D, H, W)"""
        # the input x shape is (B, G, C, D, H, W)
        # print(f"x shape: {x.shape}")

        # separate the x and x_cls, the first D is the x_cls
        x_clses = x[:, :, :, 0, :, :] # B G C H W
        x_normal = x[:, :, :, 1:, :, :] # B G C D H W
        # print(f"x_clses shape: {x_clses.shape}")
        # print(f"x_normals shape: {x_normal.shape}")

        # Merging the normal class tokens
        # average pooling x_normal along temporal dimension D for each group
        # x_normal shape after mean is (B, G, C, 1, H, W)
        x_normal = x_normal.mean(dim=3, keepdim=False)
        # print(f"x_normal shape after mean: {x_normal.shape}")
        # Chage the shape of x_normal to (B, C, G, H, W)
        x_normal = x_normal.permute(0, 2, 1, 3, 4).contiguous()
        

        # reshape x_clses to (B, G, C, D, H, W)
        x_clses = x_clses.unsqueeze(3)
        # print(f"x_clses shape after unsqueeze: {x_clses.shape}")
        # padding the x_clses to (B, G, C, D, 14, 14)
        x_clses = self.padding(x_clses)
        # print(f"x_clses shape after padding: {x_clses.shape}")
        # get the x_cls from every windows
        x_clses = self.avgpool3d(x_clses) # B G C D' H' W'
        # print(f"x_cls_group shape after avg_poll2: {x_clses.shape}")

        # merge the x_clses
        x_cls_weighted = self.merge_cls(x_clses)
        # print(f"x_cls_weighted shape: {x_cls_weighted.shape}")

        # upsampling the x_cls
        x_cls_weighted = self.upsampling_xcls(x_cls_weighted)
        # print(f"x_cls_weighted shape after upsampling: {x_cls_weighted.shape}")

        # merge the x_cls with x_group
        # print(f"x_normal shape after permute: {x_normal.shape}")
        x_xcl_s2 = torch.cat((x_cls_weighted, x_normal), dim=2)
        # print(f"x_xcl_s2 shape: {x_xcl_s2.shape}")
...
class GEgoviT(nn.Module):
    def __init__(self,
        ...
        # bild layers
        # define the stage1s only one time
        self.stage1 = Stage1(
    ...
    def forward(self, x, features):
        ...
        for i in range(self.G):
            ...
            x_xcl_list.append(self.stage1(x_group, features_group))
...
# input = torch.randn(2, 32, 3, 120, 160)
# input_feature = torch.randn(2, 32, 3, 2048)
