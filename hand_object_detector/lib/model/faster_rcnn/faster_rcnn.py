# ------------------------------------------------------------------------------
# Modified fasterRCNN.py for the hand object detector module
...
class _fasterRCNN(nn.Module):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, box_info):
        ...
        # print(f"base_feat shape is {base_feat.shape}")
        # feed base feature map tp RPN to obtain rois
        rois, rpn_loss_cls, rpn_loss_bbox = self.RCNN_rpn(base_feat, im_info, gt_boxes, num_boxes)

        ####
        # print(f"rois in fasterrcnn.py type is ------ {rois.shape}")
        ...
        # print(f"rois is {rois.data[0][0]}")
        # print(f"rois_padded is {rois_padded.data[0][0]}")

        # do roi pooling based on predicted rois
        if cfg.POOLING_MODE == 'align':
            pooled_feat = self.RCNN_roi_align(base_feat, rois.view(-1, 5))
            pooled_feat_padded = self.RCNN_roi_align(base_feat, rois_padded.view(-1, 5))
            # print(f"pooled_feat is {pooled_feat.shape}")
            # print(f"pooled_feat_padded is {pooled_feat_padded.shape}")
        ...
        # print(f"pooled_feat2 is {pooled_feat.shape}")
        # print(f"pooled_feat_padded2 is {pooled_feat_padded.shape}")
        # compute bbox offset
        ...
        # extension layer
        # print(f"cls_score is {cls_score.shape} one content is {cls_score.data[0]}")
        # print(f"cls_prob is {cls_prob.shape} one content is {cls_prob.data[0]}")
        RCNN_loss_cls = 0
        RCNN_loss_bbox = 0
        loss_list = []
        ...
        # print(f" cls_prob2 is {cls_prob.shape}")
        # print(f" bbox_pred is {bbox_pred.shape}")


        return rois, cls_prob, bbox_pred, pooled_feat, loss_list
