...
class GazeFeaturesExtractor:
    ...
    def build_bounding_box(x, y, W, H):
        ...
        bottom_right_x = min(224, int(x + (W / 2)))
        bottom_right_y = min(224, int(y + (H / 2)))
    ...
    def save_gaze_feature(self, gaze_feature, image_name):
        # Remove the .jpg from the image_name and add .npy
        ...
        gaze_feature = gaze_feature.cpu()
        # Convert the gaze feature to a numpy array
        gaze_feature = gaze_feature.numpy()
        # Save the gaze feature as a .npy file
        np.save(path_to_save, gaze_feature)
        # print(f"Gaze feature saved to {path_to_save}")
    ...
    def concatenate_features(self):
        ...
        feature_dir = self.gazepath_tosave.replace('/gaze_new', '')
        gaze_path = os.path.join(feature_dir, "gaze_new")
        hand_path = os.path.join(feature_dir, "hand")
        object_path = os.path.join(feature_dir, "object")
        combined_features_path = os.path.join(feature_dir, "combined_features_npy")
        ...
        for file in gaze_files:
            ...
            # save the combined_features in npy file
            np.save(combined_features_path + '/' + file.split(".")[0] + ".feather", combined_features)
...
# extractor = GazeFeaturesExtractor(frames_dir="Frames3/test_split1", gazepath_tosave="Extracted_HOFeatures/test_split1/gaze_new")
